...
func impliedStructType(rt reflect.Type, path cty.Path) (cty.Type, error) {
	...
	tfs := rivertags.Get(rt)
	if len(tfs) == 0 {
		return cty.NilType, path.NewErrorf("no cty.Type for %s (no river field tags)", rt)
	}

	var (
		attrs    = make(map[string]cty.Type, len(tfs))
		optional []string
	)

	{
		...
		for _, tf := range tfs {
			...
			path[len(path)-1] = cty.GetAttrStep{Name: tf.Name}

			fieldType := rt.Field(tf.Index).Type
			attr, err := impliedType(fieldType, path)
			if err != nil {
				return cty.NilType, err
			}
			...
			if tf.IsOptional() {
				optional = append(optional, tf.Name)
			}
		}
	}
