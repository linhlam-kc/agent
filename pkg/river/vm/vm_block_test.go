...
func TestVM_Block_Children_Blocks(t *testing.T) {
	...
	t.Run("Decodes multiple instances of children blocks into an array", func(t *testing.T) {
		type block struct {
			Value    int           `river:"value,attr"`
			Children [2]childBlock `river:"child.block,block"`
		}
	...
	t.Run("Decoding into an array sets unused elements to zero value", func(t *testing.T) {
		type block struct {
			Value    int            `river:"value,attr"`
			Children [10]childBlock `river:"child.block,block"`
		}
		...
		require.Zero(t, actual.Children[5], "Unused elements should have been reset to zero value")
	})
...
func TestVM_Block_Label(t *testing.T) {
	...
	t.Run("Struct must have label field if block is labeled", func(t *testing.T) {
		...
		err := eval.Evaluate(nil, &block{})
		require.EqualError(t, err, `block "some_block" does not support specifying labels`)
	})
	...
	t.Run("Block must have label if struct accepts label", func(t *testing.T) {
		...
		err := eval.Evaluate(nil, &block{})
		require.EqualError(t, err, `block "some_block" requires non-empty label`)
	})
	...
	t.Run("Rejects label into non-string field", func(t *testing.T) {
		type block struct {
		...
		require.PanicsWithValue(t, expectPanic, func() {
			_ = eval.Evaluate(nil, &block{})
		})
	})
}
